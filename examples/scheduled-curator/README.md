...
- **Graceful degradation recorded as signal (§5(f)).** When the queue input is missing, the `intake` stage records a `fallback_used` marker and counts against the signal-failure budget rather than silently defaulting.
