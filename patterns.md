...
- **The input carries provenance** (§1 corollary (f); `state-schema-patterns.md`, "Input snapshot with provenance"): value, source, timestamp, a consistency check where one is cheap (checksum, an invariant the next stage can validate).
