...
4. **Stochastic error: outputs are noisy signals.** Every output is a noisy sample of a latent quality, not the quality itself. Even on tasks the model can do, some fraction of attempts fail: a sign slip, an off-by-one, a dropped constraint, a misread token. *Consequence:* a single pass is evidence, not a measurement.
...
- **Scripts.** Deterministic code the orchestrator or an agent invokes directly (`bash`, `python`, etc.). No model tokens consumed during execution, no coherence drift inside the call. Use for: idempotent transforms, file generation, mechanical checks, schema validation. The decision to invoke is still the model's; the work is not. Wrap in a skill when the invocation needs a trigger predicate, usage guidance, or restated invariants (see `skills-best-practices.md`, "scripts for deterministic ops; prose for judgment"); otherwise invoke directly.
