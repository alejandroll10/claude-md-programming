...
**Mode or variant flag.** When one CLAUDE.md serves multiple flows (short-form scan, full rebuild, maintenance run), the active flow is a closed-enum state field set by a trigger phrase and read before `current_stage`. Each mode has its own top-level transition table; the trigger-to-mode mapping is orchestrator-level routing and lives in CLAUDE.md, not in stage docs. Mode is set once per run: mid-run swaps are schema rewrites (§1 corollary (i)), handled as rollback-plus-new-run (§1 corollary (j)). Shared stages keep one doc each, with mode-specific preconditions as top-of-doc mode-guards; branching on `mode` inside every stage body drifts under premise 3.
