...
**Mode or variant flag.** When one CLAUDE.md serves multiple flows (a short-form scan, a full rebuild, a maintenance-only run), the active flow is a closed-enum state field. A trigger phrase picks it; the orchestrator reads `mode` before `current_stage` on each run, and each mode has its own top-level transition table. Trigger-phrase-to-mode mapping lives in CLAUDE.md as orchestrator-level routing, not in stage docs or agent definitions. Mode is set once per run, never changed mid-run: mid-run swaps are schema-level rewrites (§1 corollary (i)) and should be handled as a rollback-plus-new-run (§1 corollary (j)), not as a transition. Stages shared between modes keep one stage doc each, with mode-specific preconditions documented as mode-guards at the top of the stage doc; the stage body does not branch per mode beyond what its preflight requires. Antipattern: branching on `mode` inside every stage body puts routing in N stages, and drift in any one corrupts the rest (premise 3).
