...
- **Hard-stop on missing input (contrast with §5(f) graceful degradation).** When the queue input is absent, the `intake` stage records a `fallback_used` marker and emits `EMPTY_QUEUE`, routing to `status = stuck`. This is not a fallback-that-continues (which is what §5(f) defines); it is the discipline of *not* silently defaulting. The example does not exercise §5(f) itself.
